...
// TIMO: PCB control buttons
// Emmits a "$H" command to grbl to home all axis to a known location before doing any kind of milling
...
// Emmits a G92 X0Y0 command to reset home to the current location. Z remains untouched.
// This should not be used but can be useful when milling with an offset.
void MainWindow::resetHomeXY()
{
    QString line = QString("G92 X0 Y0").append("\r");

    emit gotoXYZFourth(line);
}

// This resets the current X-axis coordinate system to a known negative offset defined by the user.
// This is required to be able to mill the bottom side of a PCB in degative space as well as drilling and milling outlines in degative space!
// This required a fresh home position directly after "$H".
void MainWindow::setBottomCoordinateSystem()
{
...
// This emmits "G38.2 Z-10 F50" or similar to probe for Z0 with connected probing clamps to the PCB and the endmill.
// After successfull probing a "G92 Z0" command will be sent to reset Z-axis position to 0.
void MainWindow::probeZ()
{
    if(ui->ckbProbeWiresConnected->isChecked())
    {
        QString line = QString("G38.2 Z").append(ui->txtZProbingDepth->text())
                .append(" F").append(ui->txtZProbingFeedrate->text()).append("\r");
        emit gotoXYZFourth(line);

        line = QString("G92 Z0").append("\r");
        emit gotoXYZFourth(line);
    }
    else
    {
        receiveListOut("Please make sure you connected probing wires and check the checkbox to confirm that you did!");
    }

    // Unckeck false again to prevent user error
